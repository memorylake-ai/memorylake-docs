...
在[控制台](https://app.memorylake.ai)中创建 API Key（侧边栏的 **API Keys**），并确认希望 Agent 使用的项目。控制台的 **Integrations → OpenClaw** 页面会生成下面这些已预填参数的命令。
</Step>
...
独立的 **memorylake-models-openclaw** 插件会把 MemoryLake 的 [Model Router](/zh/features/model-router/overview) 注册为模型提供方——一个 `sk-…` Key 同时打通记忆与模型：
...
<CardGroup cols={2}>
<Card title="集成概览" icon="plug" href="/zh/features/memorylake/mcp-servers/overview">

...
- **文件**（`item_type: "file"`） — 注册一个字节已通过分块上传存入存储的文件。您必须先调用[创建上传会话](/zh/features/memorylake/api-reference/library/create-upload)并收集每个分块的 `ETag`。
...
**流程说明（文件模式）：** 以 `item_type: "file"` 模式调用此接口是文件库上传流程的第 2 步；第 1 步是[创建上传会话](/zh/features/memorylake/api-reference/library/create-upload)（相同权限）。如需让文件出现在项目中，请调用[导入文档](/zh/features/memorylake/api-reference/v3-documents/import-documents) — 该操作还额外需要 [`project:doc_add`](/zh/features/team-collaboration/permission-reference#向项目添加文档) 权限。
</Note>
...
1. 调用[创建上传会话](/zh/features/memorylake/api-reference/library/create-upload)并传入 `file_size`，收到 `upload_id` 以及每个分块的预签名 URL。
...
<Expandable title="from">
  ...
  [创建上传会话](/zh/features/memorylake/api-reference/library/create-upload)返回的 `upload_id`。
  </ParamField>
...
<Warning>
`overwrite` 仅替换文件库中存储的文件字节 — **不会**重新处理或重新索引此前从该文件导入的任何项目文档。要将新内容同步到项目中，请在覆盖后再次调用[导入文档](/zh/features/memorylake/api-reference/v3-documents/import-documents)（或改用一个新文件）。
</Warning>

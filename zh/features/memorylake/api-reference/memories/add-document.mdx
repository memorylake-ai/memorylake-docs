---
title: "添加文档"
description: "将一个或多个资料库文件导入项目作为文档"
---
...
将用户资料库中的文件导入项目。每个导入的文件会成为一个项目文档，并立即加入处理队列。该操作支持批量处理，且支持**部分成功** —— 响应会告知导入成功和失败的数量。
...
- `ROOT` —— 一次调用导入用户在整个资料库中有权访问的所有文件。

如果要添加的文件尚未在资料库中，请先使用[资料库上传流程](/zh/features/memorylake/api-reference/library/overview#uploading-a-file-end-to-end)上传文件。

<Note>
**所需权限（全部需要）：**

- [`project:doc_add`](/zh/features/team-collaboration/permission-reference#add-a-document-to-a-project) · `instance`
- [`drive:item_read`](/zh/features/team-collaboration/permission-reference#read-your-files) · `service`

**工作流说明：** 调用此端点时，以上两个权限会同时检查 —— 需要 `drive:item_read` 是因为服务器会解析并读取每个引用的云盘项。如果你需要先**上传新文件**，资料库上传调用（[创建上传](/zh/features/memorylake/api-reference/library/create-upload) 和 [创建项](/zh/features/memorylake/api-reference/library/create-item)）还额外需要 [`drive:item_add`](/zh/features/team-collaboration/permission-reference#upload-and-create-files) 权限。
</Note>
...
要导入的资料库项 ID。接受文件和目录 ID 的任意组合 —— 目录会递归展开为所有后代文件，不支持的项类型会被静默跳过。也接受别名 `MY_SPACE`（你的工作区根目录）和 `ROOT`（整个资料库）。
</ParamField>
...
<Steps>
<Step title="将文件放入资料库">
通过[分块上传流程](/zh/features/memorylake/api-reference/library/overview#uploading-a-file-end-to-end)上传文件，或使用[列出项](/zh/features/memorylake/api-reference/library/list-items)查找已有文件。
</Step>
<Step title="调用添加文档">

...
返回会话中消息的分页列表，按序列号排序。您可以使用 `after_message_id` 仅获取在某个已知位置之后添加的消息，这对增量同步新消息非常有用。
...
<Expandable title="分页消息列表">
  ...
  <Expandable title="消息对象">
    ...
    <Expandable title="内容块">
      <ResponseField name="block_type" type="string">块类型：`text` 或 `image`</ResponseField>

...
返回项目中会话的分页列表。使用分页令牌遍历大型结果集。

<Note>
**所需权限：** `project:conv_list`
</Note>
...
<Expandable title="分页会话列表">
  ...
  <Expandable title="会话对象">
    ...
    <ResponseField name="kind" type="string">会话类型（例如 `chat`、`support`、`interview`）</ResponseField>

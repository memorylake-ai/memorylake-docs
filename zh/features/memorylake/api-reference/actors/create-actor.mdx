...
创建一个新的 Actor。Actor 代表会话中的参与者——通常是终端用户或系统身份，其交互会生成记忆。创建 Actor 后，您可以将其[绑定到工作空间](/zh/features/memorylake/api-reference/actors/bind-actor)，使其能够参与该工作空间项目中的会话。
...
Actor 类别。常用值为 `user`（终端用户）和 `system`（自动化身份）。您可以传入任何适合您业务领域的字符串。
</ParamField>
...
<Expandable title="Actor">
  ...
  <ResponseField name="actor_type" type="string">Actor 类型</ResponseField>

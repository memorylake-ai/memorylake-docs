...
返回账户中所有 Actor 的分页列表。使用 `custom_id` 按外部标识符筛选 Actor。
...
按自定义 ID 筛选。仅返回 `custom_id` 完全匹配的 Actor。
</ParamField>
...
<Expandable title="分页 Actor 列表">
  ...
  <Expandable title="Actor 对象">
    ...
    <ResponseField name="actor_type" type="string">Actor 类型，例如 `user` 或 `system`</ResponseField>

---
title: "导入文档"
description: "将资料库中的文件作为文档导入到项目中"
---
...
将一个或多个资料库文件作为文档导入到项目中。MemoryLake 会异步处理每个文件——对其内容进行索引以用于语义搜索和记忆提取。你可以通过[获取文档](/zh/features/memorylake/api-reference/v3-documents/get-document)来查看处理进度。
...
<Note>
文件必须先存在于资料库中才能导入。请先使用[资料库 API](/zh/features/memorylake/api-reference/library/overview) 上传文件。
</Note>
...
要导入的资料库项目 ID 数组。每个 ID 必须引用资料库中已存在的文件。
</ParamField>
...
<Expandable title="导入结果">
  ...
  <Expandable title="导入详情">
    <ResponseField name="result" type="string">该文件的结果：`success`、`failure` 或 `duplicate`。</ResponseField>
    <ResponseField name="drive_item_id" type="string">已处理的资料库项目 ID。</ResponseField>

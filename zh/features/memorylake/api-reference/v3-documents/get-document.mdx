...
<Expandable title="文档详情">
  ...
  <ResponseField name="status" type="string">处理状态：`processing`、`ready` 或 `failed`。</ResponseField>
  <ResponseField name="error" type="object|null">当 `status` 为 `failed` 时的错误详情。否则为 `null`。</ResponseField>
  ...
  <ResponseField name="document_type" type="string">表示文档类型的判别符（例如 `pdf`、`docx`、`txt`）。</ResponseField>

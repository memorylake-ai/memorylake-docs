...
<Info>
只有为您的 API Key 分组启用的模型才会出现在列表中。如果您没有看到预期的模型，请联系管理员或查看[限制与前置条件](/zh/features/model-router/others/limits-and-prerequisites)。
</Info>
...
- **`owned_by`**：模型的来源或提供商
- **定价信息**：每个 token 或请求的费用（仅供参考）
- **其他元数据**：关于模型的其他信息

<Info>
模型列表中的定价信息仅供参考。实际计费根据您的使用量计算。详情请参阅[查看用量与计费](/zh/features/model-router/view-usage-and-billing)。
</Info>
...
4. **定价**：模型列表中显示的定价仅供参考。实际计费根据您的实际使用量计算。
...
2. 在[控制台](https://app.memorylake.ai/panel)中监控您的用量
3. 查看[查看用量与计费](/zh/features/model-router/view-usage-and-billing)了解计费方式

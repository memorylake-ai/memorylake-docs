...
列出团队的角色目录：三个内建角色在前（Owner、Admin、Member），本团队的自定义角色按创建时间在后。`key` 就是[创建虚拟成员](/zh/features/management-api/members/create-member)、[更新成员角色](/zh/features/management-api/members/update-member)、[创建邀请](/zh/features/management-api/invitations/create-invitation)的 `role` 字段接受的值。任何成员的密钥都可以读取。

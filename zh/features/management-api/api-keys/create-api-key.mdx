...
新密钥与创建它的那把密钥权限相同，除非你为角色更窄的[虚拟成员](/zh/features/management-api/members/create-member)签发。
...
为某个[虚拟成员](/zh/features/management-api/members/create-member)签发密钥；该密钥随后会以这个成员的身份行事，使用它的角色，产出也归属到它。只能指向虚拟成员 —— 传入真人成员会返回 `404`。除 `service_account:create` 之外，还需要 `member:manage_virtual` 权限。
</ParamField>

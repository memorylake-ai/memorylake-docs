...
- Prerequisites: Have obtained an available token in the [console](https://app.memorylake.ai/panel/token), and the corresponding group has enabled target models or task quotas.
...
- Generate tokens in the [console](https://app.memorylake.ai/panel/token) and use them as credentials for calls; available models and quotas for different tokens or token groups do not interfere with each other.
...
- Notifications and content moderation need to be enabled in the [console](https://app.memorylake.ai/panel) or configuration with correct channel information; they do not take effect if not enabled.

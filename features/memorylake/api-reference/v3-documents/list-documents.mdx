...
<Expandable title="Paginated document list">
  ...
  <Expandable title="ProjectDocumentItem">
    ...
    <ResponseField name="status" type="string">Processing status: `processing`, `ready`, or `failed`.</ResponseField>
    <ResponseField name="error" type="object|null">Error details when `status` is `failed`. `null` otherwise.</ResponseField>
    ...
    <ResponseField name="document_type" type="string">Discriminator indicating the kind of document (e.g. `pdf`, `docx`, `txt`).</ResponseField>

...
Filter by tag. Repeat the parameter (`?tags=vip&tags=cn`) or comma-separate the values (`?tags=vip,cn`) to require several tags at once -- only actors carrying every listed tag are returned. Matching is exact and case-sensitive.
</ParamField>
...
```bash Filter by tag
curl -X GET 'https://app.memorylake.ai/openapi/memorylake/api/v3/actors?tags=vip&tags=cn' \
  -H 'Authorization: Bearer sk_xxxxxx'
```

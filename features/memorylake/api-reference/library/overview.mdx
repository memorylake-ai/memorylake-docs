...
Call [Add Documents](/features/memorylake/api-reference/memories/add-document) with the `item_id` returned from the previous step.
</Step>
</Steps>
...
<Warning>
`overwrite` and `replace` are not aliases. `overwrite` keeps the existing `item_id`; `replace` issues a new one. Neither option re-processes project documents that were already imported from the file — rerun [Add Documents](/features/memorylake/api-reference/memories/add-document) if you need the new content indexed.
</Warning>

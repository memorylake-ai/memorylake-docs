...
Returns the full record for one conflict, including every memory snapshot captured at detection time and — for `m2d` conflicts — the document excerpts that contradict the memory. Document excerpts whose source document has since been removed from the project are dropped.

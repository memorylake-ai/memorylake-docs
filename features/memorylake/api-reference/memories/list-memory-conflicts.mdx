...
description: "Browse conflicts between memories, or between a memory and a project document"
---
...
When a new memory is added, MemoryLake checks whether it contradicts another memory or a file in the project. Each contradiction is recorded as a **conflict** that your application can surface to end users — typically as a "review unresolved conflicts" list, or filtered down to a specific memory you're about to display.
...
- `m2m` — between two memories.
- `m2d` — between a memory and a project document.
</ParamField>

<ParamField query="conflict_type" type="string">
Filter by kind of inconsistency: `logical` or `knowledge`.
</ParamField>
...
<Expandable title="Paginated conflict list">
  ...
  <Expandable title="Conflict">
    ...
    <ResponseField name="category" type="string">`m2m` or `m2d`</ResponseField>
    <ResponseField name="conflict_type" type="string">`logical` or `knowledge`</ResponseField>

...
<Note>
Items in `documents` and `memories` have the same shapes as [Search Documents](/features/memorylake/api-reference/memories/search-documents) and [Search Memories](/features/memorylake/api-reference/memories/search-memories) respectively — the relevant request knobs (`top_n`, `top_k`, `threshold`) carry the same meaning here.
</Note>
...
Maximum document results to return (minimum 1).
</ParamField>

<ParamField body="top_k" type="integer" default="10">
Maximum memory results to return (1–1000).
</ParamField>

...
<Expandable title="Fields" defaultOpen>
  ...
  Project the agent can both read from and write to. Extracted facts are persisted to this project; conversation history is stored at the workspace level, not in the project.
  </ParamField>
  ...
  <Expandable title="Override fields">
    ...
    Per-request capability overrides, keyed by capability name. Include a capability to turn it on for this request with the configuration you supply; omit it to keep the agent's own setting. Capability names and their fields are snake_case, unlike the camelCase fields around them. `web_search` is the only capability exposed here today — more may follow.
    <Expandable title="Capabilities">
      <ParamField body="web_search" type="object">
      Lets the agent search the web. Pass an empty object `{}` for web search on its own, or add `open_datasets` to search MemoryLake's curated Open Data collections as well.
      <Expandable title="web_search fields">
        <ParamField body="open_datasets" type="array">
        Open Data collections to search alongside the web, by identifier: `research/academic`, `clinical/trials`, `drug/database`, `company/fundamentals`, `financial/markets`, `economic/data`, `patents/ip`.
        </ParamField>
      </Expandable>
      </ParamField>
    </Expandable>

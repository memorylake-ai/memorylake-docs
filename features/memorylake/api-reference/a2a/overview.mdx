...
MemoryLake exposes two parallel sets of A2A routes, one per protocol version:

| Routes | Protocol version | Message body | Send configuration |
|---|---|---|---|
...
<Warning>
The `/v1/` path prefix refers to the older **A2A v0.3** wire format, not to protocol version 1.0. The un-prefixed routes implement the current **A2A v1.0** format. Make sure the request body shape matches the route you call — a v0.3-style body (`content`, `blocking`) is rejected by the v1.0 routes, and vice versa.
</Warning>

Both route families support the same MemoryLake extension described below.
